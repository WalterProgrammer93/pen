          ...
          <nav aria-label="breadcrumb">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    @endif
                    <form action="{{ route('asistencias/buscar') }}" method="POST" role="form">
                        @csrf
                        <div class="col-md-20 justify-content-center m-3">
                            <div class="row justify-content-center m-3">
                                ...
                                <div class="col-md-4">
                                    <form action="{{ route('asistencias/filtro') }}" method="POST" role="form">
                                  ...
                                  <form action="{{ route('asistencias/eliminar', $asistencia->id) }}" method="POST" class="form-horizontal" role="form" onsubmit="return confirmarEliminar()">
                                      ...
                                      @if(Auth::check())
                                        @if(Auth::user()->hasRole('admin'))
                                          <a href="{{ route('asistencias/editar', $asistencia->id) }}" class="btn btn-primary">Modificar</a>
                                        ...
                                        @else
                                            @if(Auth::user()->hasRole('student'))
                                              <a href="{{ route('asistencias/editar', $asistencia->id) }}" class="btn btn-primary" disabled>Modificar</a>
                    ...
                    <div class="row">
                        <div class="col-12 d-flex justify-content-center pt-2">
                            {{ $asistencias->appends(["asistencias" => $asistencias])->links() }}
                        </div>
                    </div>
                    <!--{{ $asistencias->links() }}-->
                        ...
                        <div class="form-group row mb-0">
                            <div class="col-md-12 offset-md-0">
                                @if (Auth::check())
                                  ...
                                  <a href="{{ route('asistencias/exportar') }}" class="btn btn-danger">Exportar CSV</a>
                                  @if (Auth::user()->hasRole('admin'))
                                    <a href="{{ route('asistencias/importacion') }}" class="btn btn-warning">Importar CSV</a>
                                  @else
                                    @if (Auth::user()->hasRole('student'))
                                      <a href="{{ route('asistencias/importacion') }}" class="btn btn-warning" disabled>Importar CSV</a>
                                    @else
                                      @if (Auth::user()->hasRole('teacher'))
                                        <a href="{{ route('asistencias/importacion') }}" class="btn btn-warning" disabled>Importar CSV</a>
                                      @else
